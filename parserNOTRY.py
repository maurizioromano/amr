		...
		for arg in listaArgomenti:
			if(arg not in escludiArgomenti):
				if(str(arg) == "LOCATIONamr-unknown" or str(arg) == "amr-unknown"):
					...
					stringa = stringa + tabs + ":mod (" + getPrintableAmrName(arg, amrVarList) + ")"
				elif(str(arg)[-7:] == "LOLTIME"):
					arg = str(arg)[:-7]
					amrVarList = updateAmrVarList(arg, amrVarList)
					stringa = stringa + tabs + ":time (" + getPrintableAmrName(arg, amrVarList) + ")"
				else:
					amrVarList = updateAmrVarList(str(arg), amrVarList)
...
for (m, arg, th) in qres5:
	...
	if(getName(str(m)) == "Topic"):
		...
		m = arg
		if(str(th) != "None"):
			nomeTH = getName(str(th)) + "LOLKEK"
			bkp.append((m, [(nomeTH, [])], [False]))
		else:
			bkp.append((m, [], []))
	else:
		if(str(th) != "None"):
			nomeTH = getName(str(th)) + "LOLKEK"
			bkp.append((m, [(arg, []),(nomeTH, [])], [False,False]))
		else:
